...
const SESSION_COOKIE_NAME =
  process.env.GALIAF_AUTH_SESSION_COOKIE_NAME ?? "galiaf_manager_access_token";
export const ID_TOKEN_COOKIE_NAME =
  process.env.GALIAF_AUTH_ID_TOKEN_COOKIE_NAME ?? "galiaf_manager_id_token";
export const OIDC_STATE_COOKIE_NAME =
  process.env.GALIAF_OIDC_STATE_COOKIE_NAME ?? "galiaf_manager_oidc_state";
export const OIDC_VERIFIER_COOKIE_NAME =
  process.env.GALIAF_OIDC_VERIFIER_COOKIE_NAME ?? "galiaf_manager_oidc_verifier";
export const OIDC_RETURN_TO_COOKIE_NAME =
  process.env.GALIAF_OIDC_RETURN_TO_COOKIE_NAME ?? "galiaf_manager_return_to";

export type ResolvedManagerAuth =
  | {
      kind: "dev";
      api: ApiClient;
    }
...
export async function resolveManagerAuth(): Promise<ResolvedManagerAuth> {
...
export function resolveOidcClientId(config: PublicAuthContract): string {
  return process.env.GALIAF_OIDC_CLIENT_ID ?? config.clients.managerWebClientId;
}
