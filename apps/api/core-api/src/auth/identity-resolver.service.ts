...
export class IdentityResolverService {
  ...
  private normalizePlatformRoles(input: unknown): SupportedRole[] {
    const allowed: SupportedRole[] = ["platform_admin"];

    return this.getStringArray(input).filter((role): role is SupportedRole =>
      allowed.includes(role as SupportedRole),
    );
  }
  ...
  private extractTenantMemberships(
    ...
    const fallbackRoles = this.getStringArray(rawClaims.tenant_roles).filter(
      (role): role is TenantMembership["roles"][number] =>
  ...
  private normalizeTenantMemberships(input: unknown): TenantMembership[] {
  ...
  private calculateEffectiveRoles(
    ...
    const effectiveRoles = new Set<SupportedRole>(platformRoles);
